...
from ningsih.models import Transaksi, DetailTransaksi
...
from ningsih.forms import CartAddProductForm
from ningsih.keranjang import Cart
...
def cari(request):
    detailkategori = request.GET.get('kategori')
    detailkata = request.GET.get('kata')
    if detailkategori and detailkategori != "all":
        hasilcari = Produk.objects.filter((Q(nama__icontains=detailkategori)))
    else:
        hasilcari = Produk.objects.filter(Q(nama_produk__icontains=detailkategori) | Q(nama_produk__icontains=detailkata))
    cart_product_form = CartAddProductForm()
